...
changed_files {
  ...
  // The "fips_path" group tracks all FIPS-path artifact classifier inputs used
  // by OSS posture validation gates: HCL config examples, Helm values, Markdown
  // posture documents, COMPLIANCE-EVIDENCE.md, and wording-guardrail scripts.
  // This group must NOT include all Vault Community files; it scopes only the
  // bounded FIPS-aligned artifact set.
  group "fips_path" {
    match {
      base_dir = [
        "fips-path",
        joinpath("config", "fips-path"),
        joinpath("helm", "fips-path"),
        joinpath("scripts", "fips-path"),
      ]
    }

    match {
      base_name = ["COMPLIANCE-EVIDENCE.md"]
    }

    match {
      base_name = ["FIPS-140-3-COMPLIANCE.md"]
    }

    match {
      base_name = ["fips-path-manifest.json"]
    }
  }
}
