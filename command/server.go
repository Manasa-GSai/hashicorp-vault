...
// host state or requiring root privileges.
//
...
func warnHostFIPSMode(logger hclog.Logger, fipsEnabledPath string) {
	...
	raw, err := os.ReadFile(fipsEnabledPath)
	if err != nil {
		logger.Warn("VAULT_FIPS_PATH=1: host OS FIPS mode could not be confirmed",
			"path", fipsEnabledPath,
			"status", "file unreadable or missing",
		)
		return
	}
	...
	switch val {
	case "1":
		// Host FIPS mode is enabled — no warning needed.
		return
	case "0":
		logger.Warn("VAULT_FIPS_PATH=1: host OS FIPS mode is not enabled",
			"path", fipsEnabledPath,
			"status", "fips_enabled=0",
			"remediation", "enable FIPS mode on the host kernel before running a FIPS-path Vault deployment",
		)
	default:
		logger.Warn("VAULT_FIPS_PATH=1: host OS FIPS mode could not be confirmed",
			"path", fipsEnabledPath,
