...
func parseKMS(result *[]*KMS, list *ast.ObjectList, blockName string, maxKMS int) error {
	...
	for _, item := range list.Items {
		...
		// FIPS-path enforcement: when VAULT_FIPS_PATH=1, reject explicit awskms
		// endpoint values that do not use the kms-fips.<region>.amazonaws.com format.
		// The check is string-based, offline, and requires no AWS credentials.
		if err := checkFIPSPathAWSKMSEndpoint(seal.Type, seal.Config); err != nil {
			return multierror.Prefix(err, fmt.Sprintf("%s.%s:", blockName, key))
		}
...
//     may rely on the FIPS endpoint being the SDK default in their environment.
...
func checkFIPSPathAWSKMSEndpoint(sealType string, config map[string]string) error {
	...
	if !ok || endpoint == "" {
		// No explicit endpoint — not an error; operator relies on SDK default.
		return nil
	}
	if !strings.Contains(endpoint, "kms-fips.") {
		return fmt.Errorf(
			"endpoint %q does not use the required FIPS endpoint format "+
				"kms-fips.<region>.amazonaws.com; update the endpoint to a valid "+
				"FIPS-path operation value",
			endpoint,
		)
	}
	return nil
