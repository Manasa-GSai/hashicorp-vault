...
func TestFIPSPathAWSKMS_Enabled(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:      "awskms non-FIPS endpoint rejected",
			sealType:  "awskms",
		...
		{
			name:      "awskms http non-FIPS endpoint rejected",
			sealType:  "awskms",
			config:    map[string]string{"endpoint": "http://kms.us-west-2.amazonaws.com"},
			wantErr:   true,
			errSubstr: "FIPS-path operation",
		},
		{
			name:      "awskms govcloud non-FIPS endpoint rejected",
			sealType:  "awskms",
			config:    map[string]string{"endpoint": "https://kms.us-gov-west-1.amazonaws.com"},
			wantErr:   true,
			errSubstr: "kms-fips.",
		},
		{
			name:     "awskms FIPS endpoint accepted",
			sealType: "awskms",
			config:   map[string]string{"endpoint": "https://kms-fips.us-east-1.amazonaws.com"},
			wantErr:  false,
		},
		{
			name:     "awskms govcloud FIPS endpoint accepted",
			sealType: "awskms",
			config:   map[string]string{"endpoint": "https://kms-fips.us-gov-west-1.amazonaws.com"},
			wantErr:  false,
		},
		{
			name:     "awskms no explicit endpoint allowed",
			sealType: "awskms",
			config:   map[string]string{},
			wantErr:  false,
		},
		{
			name:     "non-awskms seal not checked",
			sealType: "gcpckms",
			config:   map[string]string{"endpoint": "https://cloudkms.googleapis.com"},
			wantErr:  false,
		},
		{
			name:      "error message includes offending endpoint",
			sealType:  "awskms",
			config:    map[string]string{"endpoint": "https://kms.eu-central-1.amazonaws.com"},
			wantErr:   true,
			errSubstr: "https://kms.eu-central-1.amazonaws.com",
		},
	}
