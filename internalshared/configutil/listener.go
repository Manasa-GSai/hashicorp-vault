...
func checkFIPSPathTLSSettings(tlsMinVersion, tlsCipherSuitesRaw string) error {
	...
	// Validate tls_min_version.
	switch strings.ToLower(strings.TrimSpace(tlsMinVersion)) {
	case "tls12", "tls13":
		// Acceptable minimum version.
	case "":
		return errors.New("FIPS-path operation requires tls_min_version to be set to tls12 or tls13; tls_min_version is missing or empty")
	case "tls10", "tls11":
		return fmt.Errorf("FIPS-path operation rejects tls_min_version %q; set tls_min_version to tls12 or tls13", tlsMinVersion)
	default:
		return fmt.Errorf("FIPS-path operation: unrecognized tls_min_version %q; set tls_min_version to tls12 or tls13", tlsMinVersion)
	}

	// Validate tls_cipher_suites is present and non-empty.
	rawTrimmed := strings.TrimSpace(tlsCipherSuitesRaw)
	if rawTrimmed == "" {
		return errors.New("FIPS-path operation requires tls_cipher_suites to be explicitly set; tls_cipher_suites is missing or empty")
	}

	// Validate each suite name is in the approved allow-list.
	for _, suite := range strings.Split(rawTrimmed, ",") {
...
func (l *Listener) parseTLSSettings() error {
	...
	// FIPS-path TLS validation: fail closed when VAULT_FIPS_PATH=1 and the
	// listener does not meet minimum TLS requirements for the posture.
	// This check runs before the raw cipher suite value is cleared so we can
	// compare suite names against the approved allow-list.
	if err := checkFIPSPathTLSSettings(l.TLSMinVersion, l.TLSCipherSuitesRaw); err != nil {
		return err
	}
