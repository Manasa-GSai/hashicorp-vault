...
const (
	...
	KeyType_AES128_CBC
	KeyType_AES256_CBC
	// KeyType_X25519 and KeyType_FF3_1 are defined here so the FIPS-path gate
	// can reject them if they are requested while VAULT_FIPS_PATH=1. Neither
	// type has a production key-generation path in the current release; any
	// use outside the FIPS-path gate returns an unsupported key type error.
	KeyType_X25519
	KeyType_FF3_1
...
// X25519: ECDH with P-256/P-384 are FIPS-approved; X25519 (Curve25519 ECDH) is not.
// FF3-1: AES-FF1/FF3 is partially approved; FF3-1 specifically has known weaknesses excluded from guidance.
var fipsPathDisallowedKeyTypes = []KeyType{
	KeyType_ChaCha20_Poly1305,
	KeyType_ED25519,
	KeyType_X25519,
	KeyType_FF3_1,
...
func checkFIPSPathKeyType(kt KeyType) error {
	...
	for _, disallowed := range fipsPathDisallowedKeyTypes {
		if kt == disallowed {
			return fmt.Errorf("FIPS-path operation rejects key type %q: algorithm is not in the FIPS-Approved set; use an approved key type (aes128-gcm96, aes256-gcm96, ecdsa-p256, ecdsa-p384, ecdsa-p521, rsa-2048, rsa-3072, rsa-4096)", kt.String())
		}
	}
	return nil
}
...
func (p *Policy) RotateInMemoryWithAlgorithm(randReader io.Reader, keyType KeyType, config *KeyConfig) (retErr error) {
	if err := checkFIPSPathKeyType(keyType); err != nil {
		return err
	}
