...
var allTestKeyTypes = []KeyType{
	...
	// KeyType_X25519 and KeyType_FF3_1 are reserved for FIPS-path gate enforcement;
	// they have no production key-generation path in the current release.
	KeyType_X25519, KeyType_FF3_1,
}
...
func TestFIPSPathKeyType_Enabled(t *testing.T) {
	...
	disallowedCases := []struct {
		...
		{KeyType_ChaCha20_Poly1305, "chacha20-poly1305"},
		{KeyType_ED25519, "ed25519"},
		{KeyType_X25519, "x25519"},
		{KeyType_FF3_1, "ff3-1"},
	}
	...
	for _, gateValue := range []string{"", "0"} {
		...
		t.Run("VAULT_FIPS_PATH="+gateValue, func(t *testing.T) {
			...
			for _, kt := range disallowedTypes {
				err := checkFIPSPathKeyType(kt)
				if err != nil {
					t.Fatalf("VAULT_FIPS_PATH=%q: expected nil error for key type %q, got %v", gateValue, kt.String(), err)
				}
			}
		})
	}
...
func TestFIPSPathKeyType_Allowed(t *testing.T) {
	...
	for _, kt := range allowedTypes {
		...
		t.Run(kt.String(), func(t *testing.T) {
			err := checkFIPSPathKeyType(kt)
			if err != nil {
				t.Fatalf("VAULT_FIPS_PATH=1: expected nil error for approved key type %q, got %v", kt.String(), err)
			}
		})
	}
}
